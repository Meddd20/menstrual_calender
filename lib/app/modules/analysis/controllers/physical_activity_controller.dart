...
class PhysicalActivityController extends GetxController {
  ...
  late Data data;
  RxMap<String, dynamic> physicalActivity = RxMap<String, dynamic>();
  ...
  RxMap<String, dynamic> specificPhysicalActivityData =
  ...
  void onInit() {
    ...
    physicalActivity = RxMap<String, dynamic>();
  ...
  Map<String, dynamic> specifiedDataByDate() {
    DateTime now = DateTime.now();
    specificPhysicalActivityData.clear();

    List<MapEntry<String, dynamic>> filteredData =
        physicalActivity.entries.where((entry) {
    ...
    for (var entry in filteredData) {
      specificPhysicalActivityData[entry.key] = entry.value;
    }

    return specificPhysicalActivityData;
  }

  Future<void> fetchOthers() async {
    try {
      DailyLogTags? result =
          await logRepository.getLogsByTags("physical_activity");

      if (result != null && result.data != null) {
        data = result.data!;

        physicalActivity.value = data.logs;
