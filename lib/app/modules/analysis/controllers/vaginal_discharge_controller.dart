...
class VaginalDischargeController extends GetxController {
  final ApiService apiService = ApiService();
  late final LogRepository logRepository = LogRepository(apiService);
  late Data data;
  RxMap<String, dynamic> vaginalDischarge = RxMap<String, dynamic>();
  ...
  void onInit() {
    ...
    vaginalDischarge = RxMap<String, dynamic>();
  ...
  Map<String, dynamic> specifiedDataByDate() {
    DateTime now = DateTime.now();
    specificVaginalDischargeData.clear();

    List<MapEntry<String, dynamic>> filteredData =
        vaginalDischarge.entries.where((entry) {
    ...
    for (var entry in filteredData) {
      specificVaginalDischargeData[entry.key] = entry.value;
    }

    return specificVaginalDischargeData;
  }

  Future<void> fetchOthers() async {
    try {
      DailyLogTags? result =
          await logRepository.getLogsByTags("vaginal_discharge");
