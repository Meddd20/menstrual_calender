...
class BleedingFlowView extends GetView<BleedingFlowController> {
  const BleedingFlowView({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    Get.put(BleedingFlowController());
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Bleeding Flow'),
          centerTitle: true,
          leading: IconButton(
          ...
          return Padding(
            ...
            child: Column(
              children: [
                ...
                Text(
                  "${controller.specificBleedingFlowData.length} Entries",
                  style: TextStyle(
                    color: Colors.black,
                    fontSize: 30,
                ...
                Expanded(
                  child: Container(
                    height: Get.height,
                    child: Obx(() {
                      if (controller.specificBleedingFlowData.isEmpty) {
                        return Center(child: Text("No data available"));
                      } else {
                        return ListView.builder(
                          itemCount: controller.specificBleedingFlowData.length,
                          itemBuilder: (context, index) {
                            final MapEntry<String, dynamic> entry = controller
                                .specificBleedingFlowData.entries
                                .elementAt(index);
